  ...
  const openDocumentAction = (path: string, item: HistoryItem | null) => {
    try {
      sessionStorage.setItem(
        "pendingDocument",
        JSON.stringify({ html: item?.html || "", fileName: item?.fileName || "" })
      );
      router.push(path);
    } catch (error) {
      console.error("Failed to prepare pending document:", error);
    }
  };
              ...
              >
                <div className="flex justify-between items-center p-4">
                  ...
                  <div className="flex space-x-2 ml-4 flex-shrink-0">
                    <button
                      onClick={() => openDocumentAction("/merge", item)}
                    ...
                    <button
                      onClick={() => openDocumentAction("/download", item)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-b bg-gray-50 sticky top-[56px] z-10">
              <button
                onClick={() => openDocumentAction("/merge", selectedItem)}
                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center"
              >
                <AiOutlineMergeCells className="mr-2" /> Merge
              </button>
              <button
                onClick={() => openDocumentAction("/download", selectedItem)}
